class Body {
  ...
  Body(float r, float d, float o) {
    ...
    randG = 255;
    randB = 255;
    randR = 255;//random(255);
    randG = 255;//random(255);
    randB = 255;//random(255);
  }
  
  void colorChange() {
    t = t + .01; // changing time by step .01
    float r = 0;//noise(t); // getting values of perlin noise for different points in time
    float g = 0;//noise(t+1);
    float b = 0;//noise(t+2);
  ...
  void spawnChildren(int total, int level) { // INITIALIZING RECURSIVE OBJECTS
    ...
    for (int i = 0; i < bodies.length; i++) {
      ...
      float o = random(-0.03, 0.03)*level; // some orbit speed value between neg and pos .06
